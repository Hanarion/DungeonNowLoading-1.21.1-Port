...
public class WhiteShockwaveParticle extends TextureSheetParticle {
    private SpriteSet spriteSet;
    private int axis;
    private float degree;
    private float scale;

    protected WhiteShockwaveParticle(ClientLevel clientLevel, double x, double y, double z, int axis, float degree, float scale, SpriteSet spriteSet) {
    ...
    public void render(VertexConsumer vertexConsumer, Camera camera, float partialTick) {
        ...
        if (progress < fadeStart) {
            ...
            this.alpha = startAlpha - Mth.clamp(fadeProgress, 0.0f, startAlpha);
        }
    ...
    public static class Factory implements ParticleProvider<ScalableAxisParticleType.ScalableAxisParticleData> {
        ...
        @Nullable
        public Particle createParticle(ScalableAxisParticleType.ScalableAxisParticleData data, ClientLevel clientLevel, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            WhiteShockwaveParticle particle = new WhiteShockwaveParticle(clientLevel, x, y, z, data.getAxis() , data.getDegree(), data.getScale(), this.sprites);
