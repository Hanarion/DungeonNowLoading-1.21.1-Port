...
/**
 * Minimal cube blob renderer for PayloadEntity. Renders a small tinted cube with kind-based colors.
 */
public class PayloadEntityRenderer extends EntityRenderer<PayloadEntity> {
    ...
    public void render(PayloadEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        ...
        switch (entity.getKind()) {
            case CRYO -> { cr = 255; cg = 255; cb = 255; }
            case OIL -> { cr = 0; cg = 0; cb = 0; }
            case CALTROP -> { cr = 128; cg = 128; cb = 128; } // gray
            default -> { cr = 255; cg = 255; cb = 255; }
        }

        // Use entity cutout (no cull) with white texture; force fullbright and no overlay to avoid tint skew
        VertexConsumer vc = buffer.getBuffer(RenderType.entityCutoutNoCull(BLANK));
        int light = LightTexture.FULL_BRIGHT; // 0xF000F0
